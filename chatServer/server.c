            ...
            if (FD_ISSET(i, &cfds) && i == serverFD) {
                ...
                for (x = 0; x < 10; x++) {
                    if (clientList[x].signedIn == 0) {
                        ...
                        snprintf(content, sizeof(content) + 1, "%s%s", clientList[x].name,
                ...
                if (n == 0) {
                    ...
                    snprintf(option, sizeof(option) + 1, "SERVER");
                    snprintf(content, sizeof(content) + 1, "%s%s", currentClient->name,
                    ...
                    switch (packet[0]) {
                        ...
                        case 'c'    :
                            ...
                            snprintf(content, sizeof(content) + 1,
                                     "--COMMAND LIST--\ncommand - description : usage\n"
                                             ...
                                             "/d - Disconnect: /d\n"
                                             "/e - Exit Room and return to general chat: /e\n"
                                             "/f - Send jpg to specific user: /f username FILEPATH\n"
                                             "/g - Send jpg to Room: /f FILEPATH\n"
                                             "/h - List of users in current room: /h\n"
                                             "/i - Send file of any type to room: /i FILEPATH (UPDATED PROTOCOL)"
                                             "/l - List all users in all rooms: /l\n"
                                             "/n - Name registration: /n NewName\n"
                                             "/r - Send message to current room: /r message\n"
                                             "/s - Switch rooms: '/s RoomName' OR just '/s' to see a list of rooms\n"
                        ...
                        case 'e'    :    //commands.exitRoom(i);
                            ...
                            for (y = 29; y < sizeof(packet); y++)
                                packet[y] = content[y - 29];
                            for (y = 0; y < 10; y++)
                                if (clientList[y].signedIn &&
                                    clientList[y].currentRoom == currentClient->currentRoom &&
                                    clientList[y].socket != currentClient->socket)
                                    write(clientList[y].socket, packet, sizeof(packet));
                            bzero(&option, sizeof(option));
                            bzero(&content, sizeof(content));
                            bzero(&packet, sizeof(packet));
                        ...
                        case 'f'    :    //commands.pFile(clinetList,packet);private file
                            ...
                            if (!sent) {
                                snprintf(option, sizeof(option) + 1, "SERVER");
                                snprintf(content, sizeof(content) + 1,
                                         "User does not exist: type '/l' for a list of current users.");
                                for (x = 1; x < 21; x++)
                                    packet[x] = option[x - 1];
                                for (x = 29; x < sizeof(packet); x++)
                                    packet[x] = content[x - 29];
                        ...
                        case 'g'    :
                            ...
                            for (x = 0; x < 10; x++)
                                if (clientList[x].signedIn &&
                                    clientList[x].currentRoom == currentClient->currentRoom)
                                    write(clientList[x].socket, packet, sizeof(packet));
                            break;
                            ...
                            for (x = 0; x < 10; x++)
                                if (clientList[x].name[0] != '\0') {
                                    sprintf(temp, "%s is in room: %s\n", clientList[x].name,
                                            rooms[clientList[x].currentRoom]);
                                    strcat(content, temp);
                                }
                        ...
                        case 'n'    :    //name registration
                            ...
                            if (packet[1] == '\0') {
                                ...
                                snprintf(content, sizeof(content) + 1, "%s",
                                         "You did not enter a name. Try: '/n newName'");
                                for (x = 1; x < 21; x++)
                                    packet[x] = option[x - 1];
                                for (x = 29; x < sizeof(packet); x++)
                                    packet[x] = content[x - 29];
                            ...
                            {
                                if (clientList[x].signedIn &&
                                    strcmp(option, clientList[x].name) == 0) //if new name is the same
                                {
                                    snprintf(content, sizeof(content) + 1, "%s%s%s", "The name '", option,
                            ...
                            if (!conflict) {
                                snprintf(content, sizeof(content) + 1, "%s%s%s", currentClient->name,
                                         " has changed their name to ", option);
                                for (x = 0; x < 20; x++)
                        ...
                        case 's'    :    //switch rooms
                            switch (packet[1]) {
                                ...
                                default     :
                                    snprintf(option, sizeof(option) + 1, "SERVER");
                                    snprintf(content, sizeof(content) + 1,
                                             "Valid rooms are: 'X', 'Y', and '1'. Exit a room with '/e'.");
                                    for (y = 1; y < 21; y++)
                                        packet[y] = option[y - 1];
                                    for (y = 29; y < sizeof(packet); y++)
                                        packet[y] = content[y - 29];
                        ...
                        default        :
                            snprintf(option, sizeof(option) + 1, "SERVER");
                            snprintf(content, sizeof(content) + 1,
                                     "This server does does not implement the sent command.");
                            for (x = 1; x < 21; x++)
                                packet[x] = option[x - 1];
                            for (x = 29; x < sizeof(packet); x++)
                                packet[x] = content[x - 29];
